...
final_genotype_fam=$4 # as in /nfs/DATA/BIOCARD/GWAS/Genotyped/Cleaned/BIOCARD_genotyped_final.fam and so on... 

# Arguments:
## cohort_stem - this is whatever the name of your cohort is and possibly what version of imputation this is
## imputation_results_folder - this is whatever folder the imputation results folder have been downloaded and unzipped in
## cleaned_imputed_folder - this is whatever folder you want to write the cleaned imputed files to
## final_genotype_file - this is the name of the fam file right before pre-imputation QC from which to pull sex information from
...
for i in $(seq 1 22); do
    ...
    plink2 --vcf chr${i}.dose.vcf.gz --exclude-if-info "R2<0.8" --make-bed --out chr${i}_temp
...
plink --bfile ${cohort_stem} --update-sex ${final_genotype_fam} 3 --update-name /home/clarklc/Programs/HRC_snps_nodups --make-bed --out ${cleaned_imputed_folder}/${cohort_stem}_names

cd $cleaned_imputed_folder

# SNP filters
plink --bfile ${cohort_stem}_names --maf 0.01 --geno 0.05 --hwe 0.000001 --make-bed --out ${cohort_version}_maf01_geno05_hwe6
