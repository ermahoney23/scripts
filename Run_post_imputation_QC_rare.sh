...
if [ -z "$QC_version" ];
    then
    printf "No QC version specified (e.g. allraces). Assuming that this is the rare variant pipeline in Caucasians only.\n"
    cohort_version="$cohort"
    imputation_results_folder=$( printf "Caucasians_only" )
else
    printf "Running the rare variant, $QC_version version of $cohort post-imputation QC."
...
for i in $(seq 1 22); do
    ...
    plink2 --vcf chr${i}.dose.vcf.gz --exclude-if-info "R2<0.8" --make-bed --out chr${i}_temp_rare --threads 8
...
plink --bfile chr1_temp_rare_nodups --merge-list merge_list.txt --make-bed --out ${cohort_version}_rare --threads 8

#update SNP names and add sex back into fam file
plink --bfile ${cohort_version}_rare --update-sex /nfs/DATA/${cohort}/GWAS/Genotyped/Cleaned/${cohort_version}_genotyped_final.fam 3 --update-name /home/clarklc/Programs/HRC_snps_nodups --make-bed --out /nfs/DATA/${cohort}/GWAS/Imputed/Cleaned/QC/${cohort_version}_rare_names
...
# SNP filters
plink --bfile ${cohort_version}_rare_names --maf 0.005 --geno 0.05 --hwe 0.000001 --make-bed --out ${cohort_version}_rare_maf005_geno05_hwe6
